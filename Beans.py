...
def print_results(nn_classes, nn_distances, k):
    for i, (predicted_class, distances) in enumerate(zip(nn_classes, nn_distances)):
        for j, distance in enumerate(distances):
            print(f'Vizinho {j+1}: Classe: {predicted_class}, Distância: {distance}, usando {k} vizinhos')
...
print("\nAcurácia do modelo NN1:", accuracy_score(y_test[:100], y_pred_nn1_onehot))
print("Acurácia do modelo NN2:", accuracy_score(y_test[:100], y_pred_nn2_onehot))
print("Acurácia do modelo NN3:", accuracy_score(y_test[:100], y_pred_nn3_onehot))